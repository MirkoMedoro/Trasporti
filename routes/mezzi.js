...
r.post('/', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO mezzi (azienda_id,nome,targa,lunghezza,larghezza,altezza,portata)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [req.utente.azienda_id, m.nome, m.targa, m.lunghezza, m.larghezza, m.altezza, m.portata]);
    res.json(rows[0]);
  } catch (e) { next(e); }
});
...
r.put('/:id', async (req, res, next) => {
  try {
    ...
    if (errore) return res.status(400).json({ errore });
    const { rows } = await pool.query(
      `UPDATE mezzi SET nome=$1,targa=$2,lunghezza=$3,larghezza=$4,altezza=$5,portata=$6
       WHERE id=$7 AND azienda_id=$8 RETURNING *`,
      [m.nome, m.targa, m.lunghezza, m.larghezza, m.altezza, m.portata, Number(req.params.id), req.utente.azienda_id]);
